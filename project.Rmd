...
We will first try the decision trees to fit our data. We will preprocess to impute uncollected data, center and scale.  
```{r tree, echo=TRUE, cache=TRUE, message=FALSE, fig.height=4, fig.width=6}
modTree <- train(classe ~., method="rpart", preProcess=c("knnImpute","center", "scale"), data=train1, allowParallel=TRUE)
...
Let's train our data with Random Forest method. We will preprocess to impute uncollected data, center and scale.   
```{r rForest, echo=TRUE, cache=TRUE}
modRF <- train(classe ~.,method="rf", preProcess=c("knnImpute","center", "scale"), data=train1, allowParallel=TRUE)
modRF$finalModel
```
Let's check the accuracy of this method:  
...
Let's train our data with Generalized Boosted Model method. We will preprocess to impute uncollected data, center and scale.   
```{r gbm, echo=TRUE, cache=TRUE}
modGBM <- train(classe ~.,method="gbm", preProcess=c("knnImpute","center", "scale"), data=train1, allowParallel=TRUE)
modGBM$finalModel
```
Let's check the accuracy of this method:  
